...
import {
  ...
  RefreshCw, Save, Search, Send, Server, Settings2, ShieldCheck, Trash2, Users, X, ExternalLink, ArrowLeftRight, KeyRound, Copy,
} from "lucide-react";
import {
...
function passwordPolicyError(password) {
  if (password.length < 12 || !/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/\d/.test(password) || /^[A-Za-z0-9]+$/.test(password))
    return "至少 12 位，且包含大写字母、小写字母、数字和符号。";
  return "";
}
...
function Modal({ title, description, children, onClose, wide = false, className = "" }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className={`modal ${wide ? "modal-wide" : ""} ${className}`} role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}><div className="modal-heading"><div><h2>{title}</h2>{description && <p>{description}</p>}</div><button type="button" className="icon-button" onClick={onClose} aria-label="关闭"><X /></button></div>{children}</section></div>;
}
...
function AssetsPage({ hosts, saveHost, deleteHost, openHost, user }) {
  const [query, setQuery] = useState(""); const [form, setForm] = useState(null); const [enrollment, setEnrollment] = useState(null); const [state, setState] = useState({ saving: false, error: "" }); const canManage = isAdmin(user);
  ...
  const issueEnrollment = async (host) => { setState({ saving: true, error: "" }); try { const result = await requestJson(`/api/hosts/${encodeURIComponent(host.id)}/enrollment-token`, { method: "POST" }); setEnrollment(result); setState({ saving: false, error: "" }); } catch (error) { setState({ saving: false, error: error.message }); } };
  return <><div className="page-title"><div><h1>资产管理</h1><p>按服务器组、机房和业务系统维护受监控资产。</p></div>{canManage && <button className="primary-button" onClick={() => { setState({ saving: false, error: "" }); setForm({ id: "", ip: "", group: groups[0] || "默认组", room: "", service: "", isNew: true }); }}><Plus size={16} />添加服务器</button>}</div><div className="filters"><label><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索主机名、IP、服务器组、机房或业务系统" /></label><button className="subtle-button" onClick={() => setQuery("")}>重置</button></div>{state.error && !form && <p className="form-error">{state.error}</p>}<HostTable hosts={shown} onOpenHost={openHost} manage={canManage} onEdit={(host) => { setState({ saving: false, error: "" }); setForm({ ...host, originalName: host.id }); }} onEnroll={issueEnrollment} onDelete={deleteHost} />{form && <Modal title={form.isNew ? "添加服务器" : `编辑服务器 · ${form.id}`} description="保存后将立即用于资产分组、监控和通知策略匹配。" onClose={() => setForm(null)}><form className="modal-form" onSubmit={submit}><label>服务器名称<input required autoFocus value={form.id} onChange={(event) => setForm({ ...form, id: event.target.value })} /></label><label>IP 地址<input required value={form.ip} onChange={(event) => setForm({ ...form, ip: event.target.value })} /></label><label>服务器组<input required list="server-groups" value={form.group} onChange={(event) => setForm({ ...form, group: event.target.value })} /><datalist id="server-groups">{groups.map((group) => <option key={group} value={group} />)}</datalist></label><label>机房<input required value={form.room} onChange={(event) => setForm({ ...form, room: event.target.value })} /></label><label>业务系统<input required value={form.service} onChange={(event) => setForm({ ...form, service: event.target.value })} /></label>{state.error && <p className="form-error">{state.error}</p>}<div className="modal-actions"><button className="primary-button" disabled={state.saving}>{state.saving ? "正在保存…" : "保存"}</button><button type="button" className="subtle-button" onClick={() => setForm(null)}>取消</button></div></form></Modal>}{enrollment && <EnrollmentTokenModal enrollment={enrollment} onClose={() => setEnrollment(null)} />}</>;
}

function EnrollmentTokenModal({ enrollment, onClose }) {
  const [copied, setCopied] = useState(false);
  const copy = async () => { try { await navigator.clipboard.writeText(enrollment.token); setCopied(true); } catch { setCopied(false); } };
  return <Modal title={`Agent 安装码 · ${enrollment.hostName}`} description="此安装码只显示一次。请在目标服务器的 Agent 配置向导中粘贴使用。" onClose={onClose}><div className="modal-form"><label>一次性安装码<input readOnly value={enrollment.token} onFocus={(event) => event.target.select()} /></label><p className="settings-note">有效至：{formatTime(enrollment.expiresAt)}</p><div className="modal-actions"><button className="primary-button" onClick={copy}><Copy size={16} />{copied ? "已复制" : "复制安装码"}</button><button className="subtle-button" onClick={onClose}>关闭</button></div></div></Modal>;
}
...
function UserDrawer({ user, onClose, onSave }) {
  ...
  const passwordError = form.password ? passwordPolicyError(form.password) : form.isNew ? "请设置初始密码。" : "";
  const submit = async (event) => { event.preventDefault(); if (passwordError) { setState({ saving: false, error: passwordError }); return; } setState({ saving: true, error: "" }); try { await onSave(form); } catch (error) { setState({ saving: false, error: error.message }); } };
  return <Modal className="account-modal" title={form.isNew ? "添加账号" : `编辑账号 · ${form.username}`} description={form.isNew ? "创建后请通过受控方式交付初始密码。" : "角色变更或停用会使该账号的现有会话失效。"} onClose={onClose}><form className="modal-form account-form" onSubmit={submit}><div className="modal-grid"><label>账号<input required disabled={!form.isNew} autoFocus value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} /></label><label>角色<select value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value })}>{Object.entries(roleLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label></div>{!form.isNew && <label className="toggle-field"><span><b>账号状态</b><small>停用后该账号无法继续登录。</small></span><button type="button" aria-label="切换账号状态" className={`toggle ${form.enabled ? "on" : ""}`} onClick={() => setForm({ ...form, enabled: !form.enabled })}><i /></button></label>}<label>{form.isNew ? "初始密码" : "重设密码（留空表示不修改）"}<input required={form.isNew} autoComplete="new-password" minLength="12" type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} /></label><p className={`password-help ${passwordError && form.password ? "invalid" : ""}`}>{form.password ? passwordError || "密码格式符合要求。" : "至少 12 位，包含大写字母、小写字母、数字和符号。"}</p>{state.error && <p className="form-error">{state.error}</p>}<div className="modal-actions"><button className="primary-button" disabled={state.saving || Boolean(passwordError)}>{state.saving ? "正在保存…" : "保存"}</button><button type="button" className="subtle-button" onClick={onClose}>取消</button></div></form></Modal>;
}
