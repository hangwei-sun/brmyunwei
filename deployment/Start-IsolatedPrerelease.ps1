...
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf) -or -not (Test-Path -LiteralPath $executable -PathType Leaf)) {
...
try {
    ...
    Remove-Item Env:Authentication__BootstrapAdmin__Enabled, Env:Authentication__BootstrapAdmin__Username, Env:Authentication__BootstrapAdmin__Password -ErrorAction SilentlyContinue
}
